...
def process_images(images, image_processor, model_cfg):
    ...
    if image_aspect_ratio == 'pad':
        for image in images:
            image = expand2square(image, tuple(int(x*255) for x in image_processor.image_mean))
            if 'VideoLlavaImageProcessor' in image_processor.__class__.__name__: #video-LLAVA
                image = image_processor.preprocess(image, return_tensors='pt')['pixel_values_images'][0]
            else:
                image = image_processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
            new_images.append(image)
    else:
        if 'VideoLlavaImageProcessor' in image_processor.__class__.__name__: #video-LLAVA
                return image_processor(image, return_tensors='pt')['pixel_values_images'][0]
        else:
            return image_processor(images, return_tensors='pt')['pixel_values']
    if all(x.shape == new_images[0].shape for x in new_images):
        new_images = torch.stack(new_images, dim=0)
...
class KeywordsStoppingCriteriaForBatch(StoppingCriteria):
    def __init__(self, keywords, tokenizer, input_ids):
        self.keywords = keywords
        self.keyword_ids = []
        self.max_keyword_len = 0
    ...
    def __call__(self, output_ids: torch.LongTensor, scores: torch.FloatTensor, **kwargs) -> bool:
        ...
        self.keyword_ids = [keyword_id.to(output_ids.device) for keyword_id in self.keyword_ids]
        for keyword_id in self.keyword_ids:
            if (output_ids[:, -keyword_id.shape[0]:] == keyword_id).all():
                return True
        outputs = self.tokenizer.batch_decode(output_ids[:, -offset:], skip_special_tokens=True)
        for keyword in self.keywords:
            is_keyword = [keyword in output for output in outputs]
            if False not in is_keyword:
                return True
        return False