...
class VideoLlavaForConditionalGeneration(VideoLlavaPreTrainedModel):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            # pixel_values_videos_ = [item.permute(1,0,2,3) for item in pixel_values_videos]
            # (
            #     input_ids,
            #     position_ids,
            #     attention_mask,
            ...
            #     vision_feature_select_strategy,
            # )
            # added by esyoon 2024-09-30-05:32:28
            if pixel_values_videos is not None:
                input_ids[input_ids == IMAGE_TOKEN_INDEX] = self.config.video_token_index
            if pixel_values_images is not None:
                input_ids[input_ids == IMAGE_TOKEN_INDEX] = self.config.image_token_index
            inputs_embeds = self.get_input_embeddings()(input_ids) # added by esyoon 2024-09-29-00:09:14
    ...
    def prepare_inputs_labels_for_multimodal(
        self, input_ids, position_ids, attention_mask, past_key_values, labels, images, vision_feature_layer, vision_feature_select_strategy
    ):
        # ====================================================================================================
        if (self.image_tower is None and self.video_tower is None) or images is None or input_ids.shape[1] == 1:
            if past_key_values is not None and (self.image_tower is not None or self.video_tower is not None) and images is not None and input_ids.shape[1] == 1:
        ...
        if getattr(images_minibatch, 'ndim', 0) == 4:  # batch consists of images, [mini_b, c, h, w]
            if self.image_tower is not None:
                # image_features_minibatch = self.image_tower(images_minibatch)  # [mini_b, l, c]
                image_features_minibatch, _ = self._get_vision_features(
                    pixel_values_images=images_minibatch,
                    pixel_values_videos=videos_minibatch,
                    vision_feature_layer=vision_feature_layer,
                    vision_feature_select_strategy=vision_feature_select_strategy,
                )
                iamge_features_minibatch = self.multi_modal_projector(image_features_minibatch) 
            else:
        ...
        if getattr(videos_minibatch, 'ndim', 0) == 5:  # batch consists of videos, [mini_b, c, t, h, w]
            # video_features_minibatch = self.video_tower(videos_minibatch)  # fake list [mini_b, t, l, c]
            # video_features_minibatch = self.video_tower(videos_minibatch).pooler_output  # fake list [mini_b, t, l, c]
            _, video_features_minibatch = self._get_vision_features(
                    pixel_values_images=None,
                    pixel_values_videos=videos_minibatch,
                    vision_feature_layer=vision_feature_layer,
                    vision_feature_select_strategy=vision_feature_select_strategy,
                )  # batch_size_vid, num_frames, channels, height, width = pixel_values_videos.shape
            video_features_minibatch = self.multi_modal_projector(video_features_minibatch)
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if num_images == 0:
                ...
                cur_input_embeds = torch.cat([cur_input_embeds_1, cur_image_features[0:0]], dim=0) # commented out by esyoon 2024-09-29-18:59:09
                # cur_input_embeds = torch.cat([cur_input_embeds_1, cur_image_features], dim=0) # added by esyoon 2024-09-29-18:59:08
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    ...
                    if cur_image_features.ndim == 1:
                        cur_image_features = cur_image_features.unsqueeze(0)
