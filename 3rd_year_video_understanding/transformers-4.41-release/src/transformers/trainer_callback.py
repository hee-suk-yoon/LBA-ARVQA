...
class TrainerState:
    ...
    def save_to_json(self, json_path: str):
        ...
        temp_list = []
        if len(temp_dict['log_history']) > 1:
            for log_item in temp_dict['log_history']:
                if log_item.__class__.__name__ == 'dict':
                    temp_list.append(log_item)
            temp_dict['log_history'] = temp_list
            # temp_dict['log_history'] = temp_dict['log_history'][1]
        json_string = json.dumps(temp_dict, indent=2, sort_keys=True) + "\n"
